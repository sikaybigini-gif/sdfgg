...
(() => {
  ...
  let save = loadSave();
  function loadSave() {
    try { return JSON.parse(localStorage.getItem(SAVE_KEY)) || { endings: {}, voOn: true }; }
    catch { return { endings: {}, voOn: true }; }
  }
  function persist() { try { localStorage.setItem(SAVE_KEY, JSON.stringify(save)); } catch {} }
  function unlockEnding(id) { if (ENDINGS[id] && !save.endings[id]) { save.endings[id] = true; persist(); } }
  ...
  function render(node) {
    ...
    const buildChoices = () => {
      ...
      node.choices.forEach((ch) => {
        ...
        b.innerHTML = `<span class="key">${n}</span>${ch.text}`;
        b.dataset.key = n; n++;
  ...
  function galleryHTML() {
    ...
    return `<div class="progress-line">KEŞFEDİLEN SONLAR: ${got} / ${keys.length}</div><div class="gallery">${badges}</div>`;
  }
  ...
  const INTENSITY = {
    ...
    light_ritual: 1, free_all: 0.9, win_true: 0.4, win_scarred: 0.5,
    death_sanity: 1, look_behind: 1, obey: 0.9, ninth_mark: 1, flash_figure: 1,
    break_seal: 1, steal_tape: 0.9,
  };
  function go(id) {
    const node = STORY[id];
    if (!node) { console.warn("eksik düğüm:", id); return; }
    state.node = id;
    if (ENDINGS[id]) unlockEnding(id);
  ...
  function puzzleLock({ answer, hint, onSolve, digits = 3 }) {
    return (wrap, msg) => {
      ...
      btn.onclick = () => {
        ...
        if (guess === answer) {
          setMsg(msg, "KİLİT AÇILDI ▸", "ok"); sfx.confirm(); tone(880, 0.3, "triangle", 0.09);
      ...
      symbols.forEach((s) => {
        ...
        b.onclick = () => {
          ...
          if (picks.length === answer.length) {
            if (picks.join("") === answer.join("")) { setMsg(msg, "SEMBOLLER HİZALANDI ▸", "ok"); sfx.confirm(); tone(880, 0.3, "triangle", 0.09); setTimeout(onSolve, 900); }
  ...
  function puzzleTune({ target, tolerance = 4, onSolve, hint }) {
    return (wrap, msg) => {
      ...
      btn.onclick = () => {
        ...
        if (dist <= tolerance) { locked = true; setMsg(msg, "GÖRÜNTÜ NETLEŞTİ ▸", "ok"); sfx.confirm(); tone(880, 0.3, "triangle", 0.09); bg.style.filter = ""; setTimeout(onSolve, 900); }
  ...
  function puzzleOrder({ tapes, answer, hint, onSolve }) {
    return (wrap, msg) => {
      ...
      btn.onclick = () => {
        ...
        if (order.join("") === answer.join("")) { setMsg(msg, "DOĞRU SIRA — KAYIT AÇILIYOR ▸", "ok"); sfx.confirm(); tone(880, 0.3, "triangle", 0.09); setTimeout(onSolve, 900); }
  ...
  const STORY = {
    ...
    start: {
      ...
      choices: [
        { text: "OYNAT ▶", to: "tape1", action: () => audioInit() },
    ...
    free_all: {
      ...
      text: "Fotoğrafı mühre yerleştirdin. Sekiz televizyon birer birer aydınlandı, kazınmış yüzler geri geldi — sonra huzurla karardılar. Sekiz izleyen özgür kaldı; giderken tek tek sana teşekkür etti. Kaset duvarı toza döndü. Sen de çıktın.",
      onEnter: () => { sfx.win(); glitch(); if (humNode) humNode.gain.gain.value = 0; },
      prompt: "— GİZLİ SON —  (SEKİZİ SERBEST BIRAKTIN)",
